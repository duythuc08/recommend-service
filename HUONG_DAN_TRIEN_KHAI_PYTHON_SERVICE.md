...
```
                   [Luc train - 3AM hoac manual]
                   ┌──────────────────┐   POST /api/train   ┌─────────────────────┐
                   │   Spring Boot     │────────────────────▶│  Python FastAPI      │
                   │   (@Scheduled)    │                     │  (Recommend Service) │
                   └──────────────────┘                     └──────────┬──────────┘
                                                                        │ predict_all_users()
                                                                        │ UPSERT batch 500
                                                                        ▼
┌─────────────┐      ┌──────────────────┐                    ┌─────────────────────┐
│   Next.js   │─────▶│   Spring Boot     │──SELECT TOP-N─────▶│  MySQL (user_pref.) │
│   (FE)      │      │   (READ path)     │◀───────────────────│  + enrich poster    │
└─────────────┘      └──────────────────┘                    └─────────────────────┘
                   [Luc hien thi goi y - khong qua Python]
```

**Luong train (3AM moi ngay hoac khi admin bam "Refresh"):**
1. Spring Boot `@Scheduled` goi `POST /api/train` sang Python
2. Python load toan bo review + activity_logs tu MySQL
3. Python build utility matrix, train `KNNWithMeans` (scikit-surprise)
4. Python goi `predict_all_users()`: du doan diem cho TOAN BO user x candidate movies
5. Python UPSERT ket qua vao bang `user_preference` (batch 500 dong/lan)
6. Spring Boot nhan response, ghi log — hoan tat

**Luong hien thi goi y (moi request cua user):**
1. FE goi `GET /recommendations` len Spring Boot
2. Spring Boot `SELECT TOP-N FROM user_preference WHERE user_id = ?` ORDER BY `predicted_score DESC`
3. Neu khong co dong nao (cold-start hoac batch chua chay) → Spring Boot fallback `PopularityService`
4. Spring Boot enrich them poster/ten phim/gio chieu tu DB roi tra ve FE
5. **Python KHONG duoc goi** trong buoc nay — chi doc DB

**Endpoint `/api/recommend` cua Python:** Van ton tai nhung chi dung de debug/test truc tiep (vi du curl mot user cu the). Spring Boot KHONG goi endpoint nay trong luong that.

**Lich train mo hinh:**
- Train 1 lan luc service khoi dong (de co model san sang ngay, ket qua upsert vao DB ngay)
- Train lai tu dong moi ngay luc **3:00 AM** qua Spring Boot `@Scheduled` → goi `POST /api/train`
...
```bash
pip install fastapi uvicorn sqlalchemy pymysql numpy pandas pydantic-settings scikit-surprise
```
...
def load_scoring_params(db: Session) -> dict[str, float]:
    """
    Doc ALPHA va S0 tu bang scoring_params (do Spring Boot tinh moi thu 2 3AM).
    Tra ve dict rong neu chua co (lan dau chay he thong).
    """
...
def load_all_excluded_movie_ids_bulk(db: Session) -> dict[str, set[int]]:
    """
    Load toan bo excluded movies cho TAT CA user trong 1 query duy nhat.
    Dung cho batch predict_all_users() - tranh N query rieng le cho tung user.
    excluded = phim co explicit review HOAC da BOOK_TICKET thanh cong.
    """
    query = text("""
...
def upsert_user_preferences(
    ...
    """
    Batch UPSERT list predictions vao bang user_preference.
    predictions: list of {user_id, movie_id, predicted_score, neighbor_count}
    Tra ve so dong da upsert.
    """
    if not predictions:
        return 0
...
def load_excluded_movie_ids(db: Session, user_id: str) -> set[int]:
    ...
    Dung rieng cho endpoint /recommend (debug/test 1 user) — predict_all_users()
    dung load_all_excluded_movie_ids_bulk() de tranh N query.
    """
    query = text("""
...
| `load_scoring_params` | Doc ALPHA va S0 tu bang `scoring_params` do Spring Boot ghi moi thu 2 3AM — Python dung de tinh implicit scoring |
| `load_all_excluded_movie_ids_bulk` | Load excluded movies cua TAT CA user trong 1 query — dung trong batch `predict_all_users()` de khong N+1 |
| `upsert_user_preferences` | UPSERT ket qua du doan vao bang `user_preference`, batch 500 dong/lan, Spring Boot se SELECT tu bang nay |
| `load_excluded_movie_ids` | Tra ve excluded cho 1 user cu the — chi dung boi endpoint `/api/recommend` (debug), khong dung trong batch |
...
def predict_ratings_for_user(
    ...
    neighbor_count = pred.details['actual_k'] (so neighbor thuc su dong gop).

    CHI predict cho user da co trong trainset; neu khong co (user moi hoan toan),
    tra ve {} de caller fallback sang cold-start.
    """
    try:
...
class ModelState:
    ...
    def train(self, db_session, use_implicit: bool | None = None) -> dict:
        ...
        Sau khi fit xong, tu dong goi predict_all_users() de ghi ket qua
        vao bang user_preference — Spring Boot se SELECT tu bang nay.
        """
        ...
        # Sau khi fit xong → du doan va ghi toan bo ket qua vao user_preference
    ...
    def predict_all_users(self, db_session) -> dict:
        """
        Sau khi train() xong, tinh prediction cho TOAN BO user co trong utility_long
        va UPSERT vao bang user_preference.

        - Khong gioi han top-N o buoc nay; Spring Boot tu ORDER BY + LIMIT khi doc.
        - User roi vao cold-start (predict tra {}) khong duoc ghi gi -> Spring Boot
          tu fallback Popularity khi SELECT khong tim thay dong cho user do.
        """
        t0 = datetime.utcnow()
        ...
        for user_id in all_user_ids:
            ...
            if k_u < settings.cold_start_min_interactions:
                continue  # cold-start: de Spring Boot fallback Popularity
            ...
            if not preds:
                continue  # cold-start: khong ghi, Spring Boot se fallback Popularity
...
class MoviePrediction(BaseModel):
    ...
    neighborCount: int
    source: str  # "cf" hoac "cold_start_popularity"
...
```
Luu y: endpoint /api/recommend nay phuc vu DEBUGGING / TESTING truc tiep.
Spring Boot KHONG goi endpoint nay trong luong that — Spring doc thang tu
bang user_preference ma Python da ghi truoc luc train.
...
4. Tinh K_u (so tuong tac cua user)
   ...
   └── K_u >= threshold     → dung CF (Surprise), lay tuple (score, neighbor_count)
                               └── Neu CF khong predict duoc gi → fallback Popularity
5. Sort theo score giam dan, lay top-N (mac dinh 3)
6. Tra JSON: {userId, recommendations: [{movieId, predictedScore, neighborCount, source}], ...}
```

`source` trong response (`"cf"` hoac `"cold_start_popularity"`) giup log biet ro nguon goc cua moi goi y — huu ich cho viec debug va cho phan danh gia/benchmark trong bao cao do an.
...
Luong khoi dong:
1. App start -> train ngay 1 lan (de co model san sang, ket qua upsert vao DB ngay)
2. Viec train lai tu dong 3AM do Spring Boot @Scheduled dieu phoi qua POST /api/train
...
**Giai thich:**
- `lifespan`: co che chuan cua FastAPI de chay code luc startup. Train model ngay khi service khoi dong → `predict_all_users()` se ghi ket qua vao `user_preference` → Spring Boot co the phuc vu request ngay ma khong can cho den 3AM.
- **Khong co APScheduler**: Viec lich 3AM do Spring Boot `@Scheduled` dam nhan (goi `POST /api/train`), khong phai Python. Giu Spring lam orchestrator duy nhat.
...
## 14. Tich hop voi Spring Boot

**Luu y quan trong:** Spring Boot KHONG goi Python khi hien thi goi y. Python chi duoc goi **luc train** (3AM hoac manual). Spring Boot doc thang tu bang `user_preference`.

### 14.1. Phan doc (RecommendationService.java — khong lien quan Python)
...
public class RecommendationService {
    ...
    public List<RecommendationItemResponse> getRecommendationsForUser(String userId) {
        ...
        // Doc thang tu bang user_preference, Python da ghi san luc train
        List<UserPreference> prefs = userPreferenceRepository
                .findTopByUserIdFetchMovie(userId, PageRequest.of(0, topN));

        if (prefs.isEmpty()) {
            // Cold-start: user chua co trong user_preference → fallback Popularity
...
### 14.2. Phan train (RecommendationTrainService.java — goi Python)
...
public class RecommendationTrainService {
    ...
    public void triggerTrain() {
        // Spring goi Python /api/train → Python train + upsert vao user_preference
        restTemplate.postForEntity(pythonServiceUrl + "/api/train", null, String.class);
    }
}
```

### 14.3. Scheduler (RecommendationTrainScheduler.java)

```java
@Component
public class RecommendationTrainScheduler {

    private final RecommendationTrainService trainService;

    @Scheduled(cron = "0 0 3 * * *")  // 3AM moi ngay
    public void scheduledTrain() {
        try {
            trainService.triggerTrain();
        } catch (Exception e) {
            log.error("[Scheduler] Goi Python /api/train loi: {}", e.getMessage());
            // Khong crash Spring — giu model cu, cho lan train tiep theo
        }
    }
}
```
...
**Luu y:**
- Dat timeout hop ly cho `RestTemplate` goi `/api/train` (vi du 5 phut) — train co the mat vai chuc giay neu data lon.
- Neu Python service chua khoi dong (503), Spring Boot ghi log canh bao va giu nguyen du lieu cu trong `user_preference` — FE van nhan duoc goi y tu data cu, khong bi loi.
